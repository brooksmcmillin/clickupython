...
class TestHTTPMethods:
    @pytest.mark.http
    def test__get_request(self) -> None:
        c = client.ClickUpClient(API_KEY)

        with pytest.raises(exceptions.ClickupClientError):
            c.__get_request("badmodel")

    @pytest.mark.http
    def test__post_request(self) -> None:
        c = client.ClickUpClient(API_KEY)

        with pytest.raises(exceptions.ClickupClientError):
            c.__post_request("badmodel", {"baddata": "baddata"})

    @pytest.mark.http
    def test__put_request(self) -> None:
        c = client.ClickUpClient(API_KEY)

        with pytest.raises(exceptions.ClickupClientError):
            c.__put_request("badmodel", {"baddata": "baddata"})

    @pytest.mark.http
    def test__delete_request(self) -> None:
        c = client.ClickUpClient(API_KEY)

        with pytest.raises(exceptions.ClickupClientError):
            c.__delete_request("badmodel")

    @pytest.mark.http
    def test___headers(self) -> None:
        c = client.ClickUpClient("API_KEY")

        headers = c.__headers(file_upload=False)
        assert headers == {
...
class TestClientLists:
    ...
    def test_get_list(self) -> None:
        ...
        assert type(result) == dict[str, Any]
    ...
    def test_create_list(self) -> None:
        ...
        assert len(result) > 0
        assert result["checklist"][0].status == "to do"
...
class TestClientFolders:
    ...
    @mock.patch("clickupython.client.API_URL", MOCK_API_URL)
    @pytest.mark.folders
    def test_create_folderless_list(self) -> None:
...
class TestClientTasks:
    ...
    @mock.patch("clickupython.client.API_URL", MOCK_API_URL)
    @pytest.mark.tasks
    def test_add_task_to_list(self) -> None:
    ...
    @mock.patch("clickupython.client.API_URL", MOCK_API_URL)
    @pytest.mark.tasks
    def test_remove_task_from_list(self) -> None:
    ...
    @mock.patch("clickupython.client.API_URL", MOCK_API_URL)
    @pytest.mark.tasks
    def test_get_team_tasks(self) -> None:
        ...
        assert isinstance(result, models.Tasks)
        assert result[0].name == "My First Task"

        assert result[0].assignees is not None
        assert result[0].assignees[0].username == "John Doe"
...
class TestClientComments:
    ...
    def test_get_task_comments(self) -> None:
        ...
        for comment in result:
            ...
            assert comment.user.id == 6341704
    ...
    @mock.patch("clickupython.client.API_URL", MOCK_API_URL)
    @pytest.mark.comments
    def test_get_chat_comments(self) -> None:
        c = client.ClickUpClient(API_KEY)
        result = c.get_chat_comments("3c")
        assert result[0].id == "459"

        assert result[0].user is not None
        assert result[0].user.id == 183

    @mock.patch("clickupython.client.API_URL", MOCK_API_URL)
    @pytest.mark.comments
    def test_create_chat_comment(self) -> None:
        c = client.ClickUpClient(API_KEY)
...
class TestClientChecklists:
    ...
    @pytest.mark.checklists
    def test_delete_checklist(self) -> None:
        c = client.ClickUpClient(API_KEY)
        result = c.create_checklist("1gu4f5g", "Test Checklist")
        assert isinstance(result, models.Checklist)
        assert result.id is not None
    ...
    @pytest.mark.checklists
    def test_delete_checklist_item(self) -> None:
        c = client.ClickUpClient(API_KEY)
        result = c.create_checklist("1gu4f5g", "Test Checklist")
        assert result.id is not None
        ...
        assert checklist_with_item.id is not None
...
class TestClientMembers:
    ...
    @mock.patch("clickupython.client.API_URL", MOCK_API_URL)
    @pytest.mark.members
    def test_get_list_comments(self) -> None:
        c = client.ClickUpClient("apikey")
        result = c.get_list_comments("12345")

        assert isinstance(result, models.Comments)
        assert result[0].id == "462"

        assert result[0].user is not None
        assert result[0].user.id == 183

        assert result[0].assignee is not None
        assert result[0].assignee.id == 183

        assert result[0].assigned_by is not None
        assert result[0].assigned_by.id == 183

        assert result[0].comment is not None
        assert result[0].comment[0].text == "List comment content"
...
class TestClientGoals:
    ...
    def test_get_goals(self) -> None:
        ...
        assert result[0].id == "e53a033c-900e-462d-a849-4a216b06d930"

        assert result[0].owners is not None
        assert result[0].owners[0].id == 182
        assert result[0].id == "05921253-7737-44af-a1aa-36fd11244e6f"
...
class TestSharedHierarchy:
    ...
    #         assert result[0].task.id == "1vwwavv"
    #         assert result[0].wid == "300702"
    #         assert result[0].user.username == "first_name last_name" @ mock.patch(
    #             "clickupython.client.API_URL", MOCK_API_URL
    #         )

    @pytest.mark.timetracking
    def test_get_single_time_entry(self) -> None:
        c = client.ClickUpClient(API_KEY)
    ...
    class TestTags:
        @mock.patch("clickupython.client.API_URL", MOCK_API_URL)
        @pytest.mark.tag
        def test_get_space_tags(self) -> None:
            c = client.ClickUpClient(API_KEY)
        ...
        @mock.patch("clickupython.client.API_URL", MOCK_API_URL)
        @pytest.mark.tag
        def test_create_space_tag(self) -> None:
            c = client.ClickUpClient(API_KEY)
...
class TestTeams:
    ...
    def test_get_teams(self) -> None:
        ...
        assert result[0].members[0].user.id == 123
...
class TestSpaces:
    ...
    def test_get_spaces(self) -> None:
        ...
        assert result[0].id == "790"

        assert result[0].statuses is not None
        assert result[0].statuses[0].status == "to do"

        assert result[0].features is not None
        assert result[0].features.due_dates.enabled is False
        assert result[0].features.time_tracking.enabled is False
...
class TesttimeTracking:
    ...
    def test_get_time_entries_in_range(self) -> None:
        ...
        #assert result[0].billable is False
